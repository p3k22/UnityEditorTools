...
namespace P3k.UnityEditorTools.Tools
{
   ...
   {
      internal static void Create(string baseName, string author, string version, string gitStandardRepo)
      {
...
if defined PKG_VERSION (
    set ""COMMIT_MSG=%COMMIT_MSG% [v%PKG_VERSION%]""
)

echo.
echo === Git Package Initializer ===
echo Repo    : %REPO_URL%
echo Message : %COMMIT_MSG%
if defined PKG_VERSION echo Package version detected: v%PKG_VERSION%
echo.
...
if errorlevel 1 (
    echo Commit failed. Check ignored or locked files.
    goto :end
)
...
if errorlevel 1 (
    echo Push failed. Check your connection and repo access.
) else (
    echo Successfully pushed to %REPO_URL% with history wiped!
)
...
if defined PKG_VERSION (
    ...
    if not ""!TAG!""==""v"" (
        ...
        rem If the tag already exists on the remote, delete it to keep remote in sync with wiped history
        git ls-remote --exit-code --tags origin ""refs/tags/!TAG!"" >nul 2>&1
        if not errorlevel 1 (
            echo Remote tag !TAG! exists, deleting on origin...
            git push --delete origin ""!TAG!"" >nul 2>&1
            if errorlevel 1 (
                echo Primary delete failed, trying alternative delete syntax...
                git push origin "":refs/tags/!TAG!"" >nul 2>&1
            )
        )

        echo Pushing tag !TAG!...
        git push origin ""refs/tags/!TAG!"" >nul
        if errorlevel 1 (
            echo Tag push failed, attempting force push...
            git push --force origin ""refs/tags/!TAG!"" >nul
            if errorlevel 1 (
                echo ERROR: Failed to push tag !TAG!.
            ) else (
                echo SUCCESS: Force-pushed tag !TAG!.
            )
        ) else (
            echo SUCCESS: Pushed tag !TAG!.
        )
    ) else (
        echo Skipping tag: empty version.
    )
)
...
if errorlevel 1 (
    ...
    echo Remote branch does not exist, will create on push...
) else (
...
if ""%REMOTE_HAS_BRANCH%""==""1"" if ""%HAS_COMMIT%""==""1"" (
    echo Syncing with remote...
    git pull --rebase origin ""%BRANCH%"" >nul 2>&1
    if errorlevel 1 (
        echo ERROR: Conflicts detected. Resolve manually.
        goto :end
    )
)
